...
def config():
    config_params = dict(compute = Compute.ComputeConfigSpec,vectormomentum=VMConfig.VMConfigSpec,optim=Optim.OptimConfigSpec,plotEvery=10,plotSlice=None,
    quiverEvery=1,outputPrefix="./LDDMM"
...
def Matching(cf,dl_moving,dl_target):
    for moving,target in dl_moving,dl_target:
        if cf['compute'].useCUDA and cf['compute'].gpuID is not None:
            ca.SetCUDADevice(cf.compute.gpuID)
        if os.path.isfile(cf['outputPrefix']+'m0.mhd'):
            print cf['outputPrefix'];
            return();
        ...
        common.Mkdir_p(os.path.dirname(cf['outputPrefix']))

        # Output loaded config
        if cf['outputPrefix'] is not None:
            cfstr = Config.ConfigToYAML(MatchingConfigSpec, cf)
            with open(cf['outputPrefix'] + "parsedconfig.yaml", "w") as f:
                f.write(cfstr)

        mType = ca.MEM_DEVICE if cf['compute'].useCUDA else ca.MEM_HOST


        I0 = common.LoadITKImage(moving, mType)
        I1 = common.LoadITKImage(target, mType)
        #ca.DivC_I(I0,255.0)
        #ca.DivC_I(I1,255.0)
        grid = I0.grid()
...
if __name__ == '__main__':
    ...
    moving_dataset= datasets.ImageFolder(root=os.path.join(data_dir,"/moving"))
    target_dataset = datasets.ImageFolder(root=os.path.join(data_dir,"/target"))
    dl_moving = torch.utils.data.DataLoader(moving_dataset,
                                               batch_size=1,                                              
                                               # shuffle=False,
                                               num_workers = 4)
    dl_target = torch.utils.data.DataLoader(target_dataset,
                                               batch_size = 1,
                                               # shuffle=True,                                             
                                               num_workers = 4)

    config_params = config()
    Matching(config_params,dl_moving,dl_target)
