...
def calculateIdx1D(length, patch_length, step):
    # this is a vector of patch starting points
...
def idx2pos(idx, image_size):
    """
    :param idx: Index into flattened 3D volume
    :param image_size: (list of 3 int): 	Size of 3D volume
    :return: Given a flattened idx, return the position in the 3D image space.
    """
...
def pos2idx(pos, image_size):
    """
    :param pos: (list of 3 int), Position in 3D volume
    :param image_size: (list of 3 int),	Size of 3D volume
    :return: Given a position in the 3D image space, return a flattened idx.
    """
    assert (len(pos) == 3)
    assert (len(image_size) == 3)
...
def idx2pos_4D(idx, image_size):
    """
    :param idx: flatterned idx for a 4D data (n_images * 3D image)
    :param image_size: (list of 3 int),	Size of 3D volume
    :return: return the position in the 4D space
    """
...
def calculatePatchIdx3D(num_image, patch_size, image_size, step_size):
    """
    :param num_image: num of images
    :param patch_size:  patch size in X
    :param image_size: size of X axes
    :param step_size:  stride size

    :return: calculate the idx of the patches for 3D dataset (n_images * 3D image),
    a vector with XYZ indices of all patches
    """
    pos_idx = [calculateIdx1D(image_size[i], patch_size[i], step_size[i]).long() for i in range(0, 3)]
    # the size as the number of all patches indices
    ...
    for x_pos in range(0, pos_idx[0].size()[0]):
        for y_pos in range(0, pos_idx[1].size()[0]):
            for z_pos in range(0, pos_idx[2].size()[0]):
                ...
                # the index of the patch in a flattened image
    ...
    while (batch_idx < flat_idx.size()[0]):
        ...
        for slices in range(0, cur_batch_size):
            ...
            input_batch[slices, 0] = moving[patch_pos[1]:patch_pos[1] + patch_size,
                                     patch_pos[2]:patch_pos[2] + patch_size, patch_pos[3]:patch_pos[3] + patch_size]
